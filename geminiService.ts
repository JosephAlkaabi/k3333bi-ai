...
export const fetchNewsAndGenerateSnap = async (category: Category): Promise<SnapArticle> => {
  const newsResponse = await ai.models.generateContent({
    ...
    متطلبات التحقق والصياغة:
    1. التحقق: تأكد من صحة الخبر من 3 مصادر موثوقة على الأقل.
    2. العنوان: صغ عنواناً "هوك" قصير ومثير جداً بأسلوب منصات التواصل.
    3. التفاصيل: اكتب الخبر بأسلوب مكثف (جملتين بحد أقصى) وبشكل مشوق.
    4. التاريخ: استخرج توقيت النشر الدقيق.

    النتيجة بصيغة JSON:
    - title: العنوان المثير بالعربية.
    - content: التفاصيل المحققة بالعربية.
    - news_date: تاريخ وتوقيت النشر.
    - image_prompt: وصف إنجليزي لصورة خلفية سينمائية 8K بجودة مذهلة وبدون أي نصوص تعبر عن موضوع الخبر بشكل رمزي أو واقعي.`,
    config: {
      tools: [{ googleSearch: {} }],
